...
**Profile** — przy pierwszym wejściu na stronę widoczny jest ekran wyboru profilu. Każda osoba może stworzyć własny profil (Imię i nazwisko, Pseudonim, Mail, Hasło) i mieć **swój prywatny CRM** — klienci i akcje jednego profilu są całkowicie niewidoczne dla innych. Logowanie odbywa się przez wybranie swojego profilu z listy i podanie hasła. Hasła są bezpiecznie haszowane (bcrypt), sesja trzymana w bezpiecznym ciasteczku (JWT, ważność 30 dni). W zakładce „Profile” w aplikacji można zobaczyć swoje dane i się wylogować / przełączyć profil.
...
|---|---|---|
| GET | `/api/auth/profiles` | publiczna lista profili (id + pseudonim) do ekranu wyboru |
| POST | `/api/auth/register` | rejestracja profilu (imie_nazwisko, pseudonim, mail, password) |
| POST | `/api/auth/login` | logowanie (profileId, password) |
| POST | `/api/auth/logout` | wylogowanie |
| GET | `/api/auth/me` | dane zalogowanego profilu |
| PUT | `/api/auth/settings` | ustaw podział prowizji agenta (45/50/55/60) |
| GET | `/api/system/status` | czy baza jest na trwałym dysku (Volume) |
| GET | `/api/backup/export` | pobierz kopię zapasową (klienci + akcje) |
| POST | `/api/backup/import` | przywróć dane z kopii zapasowej |
